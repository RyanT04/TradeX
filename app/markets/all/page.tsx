...
export default function AllMarketsPage() {
  ...
  async function fetchPopularCoins() {
    try {
      ...
      try {
        ...
        console.warn("CoinGecko network error, retrying in 15s...")
        await new Promise((resolve) => setTimeout(resolve, 15000))
        return fetchPopularCoins()
      }

      if (res.status === 429) {
        console.warn("Rate limited by CoinGecko, retrying in 15s...")
        await new Promise((resolve) => setTimeout(resolve, 15000))
        return fetchPopularCoins()
      }
  ...
  async function fetchExtraCoins(pageNum: number, append = false) {
    try {
      ...
      try {
        res = await fetch(
          `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=${PAGE_SIZE}&page=${pageNum}&sparkline=false&price_change_percentage=24h`
        )
      } catch {
      ...
      const mapped: Coin[] = data
        .filter((c: any) => !POPULAR_IDS.includes(c.id))
        .map((c: any) => ({
          id: c.id,
          symbol: c.symbol.toUpperCase(),
          name: c.name,
          ...
          total_volume: c.total_volume,
          market_cap: c.market_cap,
          bybitSymbol: `${c.symbol.toUpperCase()}USDT`,
        }))
      ...
      if (data.length < PAGE_SIZE || mapped.length === 0) setHasMore(false)
  ...
  async function searchCoins(query: string) {
    ...
    try {
      ...
      const mapped: Coin[] = data.map((c: any) => ({
        ...
        total_volume: c.total_volume,
        market_cap: c.market_cap,
        bybitSymbol: COINGECKO_TO_BYBIT[c.id] ?? `${c.symbol.toUpperCase()}USDT`,
      }))
  ...
  function handleShowLess() {
    setShowExtra(false); setExtraCoins([]); setPage(1); setHasMore(true); stopAllPolling()
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 pb-16">
      ...
      {loginPrompt && (
        <div className="fixed bottom-6 right-6 bg-black dark:bg-white text-white dark:text-black text-sm px-4 py-3 rounded-lg shadow-lg z-50">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 dark:border-gray-900 bg-gray-50 dark:bg-gray-950">
              ...
              <th className="text-right px-4 py-4 font-medium text-gray-400 cursor-pointer hover:text-black dark:hover:text-white transition-colors hidden lg:table-cell" onClick={() => handleSort("total_volume")}>
              ...
              filtered.map((coin) => {
                ...
                return (
                  <tr key={coin.id} className="border-b border-gray-50 dark:border-gray-900 hover:bg-gray-50 dark:hover:bg-gray-950 transition-colors">
                    ...
                    <td className="px-4 py-4 text-right text-gray-500 hidden md:table-cell font-mono">${formatPrice(coin.high_24h)}</td>
                    <td className="px-4 py-4 text-right text-gray-500 hidden md:table-cell font-mono">${formatPrice(coin.low_24h)}</td>
                    <td className="px-4 py-4 text-right text-gray-500 hidden lg:table-cell">{formatVolume(coin.total_volume)}</td>
                    <td className="px-4 py-4 text-right">
